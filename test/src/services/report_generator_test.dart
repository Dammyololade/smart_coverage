  ...
  group('ReportGeneratorImpl', () {
    ...
    group('generateJsonReport', () {
      test('should generate valid JSON report', () async {
        ...
        expect(jsonData, containsPair('files', isA<List>()));
        
        final files = jsonData['files'] as List;
        expect(files, hasLength(2));
        
        final firstFile = files[0] as Map<String, dynamic>;
        expect(firstFile, containsPair('sourceFile', 'lib/smart_coverage.dart'));
        expect(firstFile, containsPair('functions', isA<List>()));
        expect(firstFile, containsPair('branches', isA<List>()));
        expect(firstFile, containsPair('lines', isA<List>()));
      });

      test('should handle empty coverage data', () async {
        final outputPath = '${tempDir.path}/empty_report.json';
        
        const emptyCoverageData = CoverageData(files: [], summary: CoverageSummary(
          linesFound: 0,
          linesHit: 0,
          functionsFound: 0,
          functionsHit: 0,
      ...
      test('should include accurate summary statistics', () async {
        ...
        expect(summary['coveredFunctions'], equals(2)); // From mock data summary
    ...
    group('generateConsoleOutput', () {
      ...
      test('should format file details correctly', () {
        ...
        expect(output, contains('lib/smart_coverage.dart'));
        expect(output, contains('60.0%')); // File coverage percentage for smart_coverage.dart (3/5 = 60%)
      });
    });
    ...
    group('generateReports', () {
      test('should generate all configured report formats', () async {
        ...
        expect(await File('${tempDir.path}/coverage_report.json').exists(), isTrue);
      ...
      test('should handle single format configuration', () async {
        ...
        await reportGenerator.generateReports(mockCoverageData, singleFormatConfig);
        
        expect(await File('${tempDir.path}/index.html').exists(), isTrue);
        expect(await File('${tempDir.path}/coverage_report.json').exists(), isFalse);
      });
      ...
      test('should handle LCOV format', () async {
        ...
        expect(await File('${tempDir.path}/coverage_report.lcov').exists(), isTrue);
      });
      ...
      test('should handle unknown format gracefully', () async {
        ...
        expect(
          () => reportGenerator.generateReports(mockCoverageData, unknownFormatConfig),
          returnsNormally,
        );
      });
      ...
      test('should create output directory if it does not exist', () async {
        ...
        expect(await Directory('${tempDir.path}/nested/output').exists(), isTrue);
      });
    });
    ...
    group('performance tests', () {
      test('should handle large coverage data efficiently', () async {
        ...
        final largeCoverageData = CoverageData(
          files: existingFiles.map((filePath) => FileCoverage(
            path: filePath,
            lines: List.generate(50, (j) => LineCoverage(
              lineNumber: j + 1,
              hitCount: j % 3,
            )),
        ...
        expect(await File('${tempDir.path}/large_report/index.html').exists(), isTrue);
      });
