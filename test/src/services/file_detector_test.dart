  ...
  group('FileDetectorImpl', () {
    ...
    group('detectModifiedFiles', () {
      ...
      test('should filter only Dart files', () async {
        ...
        await File('${libDir.path}/dart_file.dart').writeAsString('class Test {}');
        ...
        final dartFiles = result.where((file) => file.endsWith('.dart')).toList();
...
''');
      ...
      test('should reject non-existent directory', () async {
        final isValid = await detector.validatePackageStructure('/non/existent/path');
        expect(isValid, isFalse);
      });

      test('should handle malformed pubspec.yaml', () async {
        await File('${tempDir.path}/pubspec.yaml').writeAsString('invalid yaml content [');
    ...
    group('getAllDartFiles', () {
      test('should find all Dart files in package', () async {
        ...
        await File('${tempDir.path}/lib/src/service.dart').writeAsString('class Service {}');
        await File('${tempDir.path}/test/main_test.dart').writeAsString('void main() {}');
        await File('${tempDir.path}/example/example.dart').writeAsString('void main() {}');
        ...
        expect(dartFiles.any((file) => file.endsWith('main_test.dart')), isTrue);
      ...
      test('should exclude build and .dart_tool directories', () async {
        ...
        await File('${tempDir.path}/build/generated.dart').writeAsString('// Generated');
        await File('${tempDir.path}/.dart_tool/package_config.dart').writeAsString('// Config');
        await File('${tempDir.path}/lib/main.dart').writeAsString('void main() {}');
    ...
    group('performance tests', () {
      test('should handle large number of files efficiently', () async {
        ...
        for (var i = 0; i < 1000; i++) {
          await File('${libDir.path}/file_$i.dart')
...
Future<void> _createMockGitRepo(Directory dir) async {
  ...
  await File('${gitDir.path}/refs/heads/main').writeAsString('dummy-commit-hash');
}