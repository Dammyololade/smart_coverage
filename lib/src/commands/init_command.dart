...
class InitCommand extends Command<int> {
  ...
  Future<int> run() async {
    ...
    if (fileExists) {
      _logger.info(
        '📝 Overwriting existing configuration file at: $outputPath',
      );
      _logger.info('');
    }
    ...
    try {
      ...
      _logger.info(
        '✅ Configuration file ${fileExists ? "updated" : "created"} successfully!',
      );
      _logger.info('');
      ...
      if (minimal) {
        _logger.info(
          'Generated a minimal configuration with essential options.',
        );
      } else {
        _logger.info(
          'Generated a full configuration with all available options.',
        );
      }
      ...
      if (!withAi) {
        _logger.info(
          '  2. Add AI configuration if needed (see comments in file)',
        );
      }
      _logger.info(
        '  ${withAi ? "2" : "3"}. Run: ${lightGreen.wrap("smart_coverage analyze")}',
      );
      _logger.info('');
      _logger.info(
        '💡 Tip: The configuration file supports environment variables.',
      );
