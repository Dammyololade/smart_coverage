...
abstract class ReportGenerator {
  ...
  Future<void> generateHtmlReport(CoverageData data, String outputPath, {bool darkMode = true});
...
class ReportGeneratorImpl implements ReportGenerator {
  ...
  @override
  Future<void> generateHtmlReport(CoverageData data, String outputPath, {bool darkMode = true}) async {
  ...
  Future<void> generateJsonReport(CoverageData data, String outputPath) async {
    final jsonData = {
      ...
      'files': data.files.map((file) => {
        ...
        'functions': <Map<String, dynamic>>[], // Empty for now as we don't have individual function data
        'branches': <Map<String, dynamic>>[], // Empty for now as we don't have individual branch data
  ...
  Future<void> generateLcovReport(CoverageData data, String outputPath) async {
    ...
    for (final file in data.files) {
      buffer.writeln('TN:');
      buffer.writeln('SF:${file.path}');
      
      // Function coverage (if available)
      if (file.summary.functionsFound > 0) {
        buffer.writeln('FNF:${file.summary.functionsFound}');
        buffer.writeln('FNH:${file.summary.functionsHit}');
      }
      ...
      buffer.writeln('LF:${file.summary.linesFound}');
      buffer.writeln('LH:${file.summary.linesHit}');
      
      // Branch coverage
      buffer.writeln('BRF:${file.summary.branchesFound}');
      buffer.writeln('BRH:${file.summary.branchesHit}');
  ...
  String generateConsoleOutput(CoverageData data) {
    ...
    buffer.writeln('\n📈 Coverage Summary:');
    buffer.writeln('  Files analyzed: ${data.files.length}');
    buffer.writeln('  Lines found: ${summary.linesFound}');
    buffer.writeln('  Lines hit: ${summary.linesHit}');
    buffer.writeln('  Line coverage: ${summary.linePercentage.toStringAsFixed(1)}%');
    
    if (summary.functionsFound > 0) {
      buffer.writeln('  Function coverage: ${summary.functionPercentage.toStringAsFixed(1)}%');
    }
    
    if (summary.branchesFound > 0) {
      buffer.writeln('  Branch coverage: ${summary.branchPercentage.toStringAsFixed(1)}%');
    }
    
    // Display file-level coverage
    if (data.files.isNotEmpty) {
      buffer.writeln('\n📁 File Coverage:');
      for (final file in data.files.take(10)) { // Show top 10 files
        final percentage = file.summary.linePercentage;
        final icon = percentage >= 80 ? '✅' : percentage >= 60 ? '⚠️' : '❌';
        buffer.writeln('  $icon ${file.path}: ${percentage.toStringAsFixed(1)}%');
      }
  ...
  @override
  Future<void> generateReports(CoverageData data, SmartCoverageConfig config) async {
    ...
    for (final format in config.outputFormats) {
      switch (format) {
        case 'html':
          ...
          await _generateLcovHtmlReport(data, config.outputDir, config.darkMode);
        case 'json':
  ...
  Future<void> _generateLcovHtmlReport(CoverageData data, String outputDir, bool darkMode) async {
...
<body>
  ...
  <p>No coverage data available</p>
    ...
    try {
      ...
      final scriptDir = _findScriptDirectory();
      final cssPath = '$scriptDir/templates/custom_dark_theme.css';
      
      // Build genhtml command
      final args = [
        tempLcovPath,
        '--output-directory', outputDir,
        '--title', 'Smart Coverage Report',
        '--show-details',
        '--legend',
      ];
  ...
  /// Find the scripts directory relative to the current package
  String _findScriptDirectory() {
    ...
    while (packageDir.path != packageDir.parent.path) {
      ...
      if (pubspecFile.existsSync()) {
        ...
        if (pubspecContent.contains('name: smart_coverage')) {
          final scriptsDir = Directory('${packageDir.path}/scripts');
    ...
    // Fallback: try to find scripts directory from current working directory
    var current = Directory.current;
    
    // Look for scripts directory in current or parent directories
